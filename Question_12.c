...
int main() {
    ...
    scanf("%d", &companies);
    // here asked for input of comapines 
    printf("Enter number of users who found at least 1 ad useful: ");
    scanf("%d", &atleastOne);

    int exactly[companies + 1];  // array created 
    ...
    for (int k = 2; k <= companies; k++) {
        ...
        //number of users who found exactly 2,3,4,... adds usefull and then adding to rhs 
    ...
    rhs += exactly1;
    exactly[1] = exactly1; // finding the value of people who liked exactly 1 add

    printf("\n--- Results ---\n");
    printf("Users who liked at least 1 ad (LHS) = %d\n", atleastOne);// printing how many users like atleat 1 add

    for (int k = 1; k <= companies; k++) {
        printf("Users who liked exactly %d ads = %d\n", k, exactly[k]);// printing number of people who liked exactly 1,2,3 etc add
    }
